...
#Catégorisation de la variable FEVG : 0 si FEVG > 45, 1 si FEVG 30-45, 2 si < 30
df$FRACTIONCAT = ifelse(df$FRACTION >= 45,0,
                     ifelse(df$FRACTION >= 30,1,2))

#Catégorisation avec FEVG sévère étant donné la présence probable d'un effet seuil
df$FEVGSEVERE = ifelse(df$FRACTIONCAT == 2,1,0)
```
...
### Test de l'homogénéité des courbes de survie : à garder ??
...
```{r}
#Garder Sodium dans les variables ?
...
cox_uni_fesevere = coxph(Surv(TEMPS,DC)~ FEVGSEVERE,
                         data=df,
                         ties = "exact")

cox_unitabac = coxph(Surv(TEMPS,DC)~TABAC,
                     data=df,
                     ties = "exact")

cox_uniHTA = coxph(Surv(TEMPS,DC)~HTA,
                   data=df,
                   ties = "exact")

cox_uniDIABETE = coxph(Surv(TEMPS,DC)~DIABETE,
                      data=df,
                      ties = "exact")

cox_unianemie = coxph(Surv(TEMPS,DC)~ANEMIE,
                      data=df,
                      ties = "exact")

cox_uniIR = coxph(Surv(TEMPS,DC)~IR,
                  data=df,
                  ties = "exact")

cox_uniSODIUM = coxph(Surv(TEMPS,DC)~SODIUM
                      + AGE
                      + SEXE
                      + FEVGSEVERE,
                      data=df,
...
cox_uniCPK = coxph(Surv(TEMPS,DC)~CREATK,
                   data=df,
                   ties = "exact")
...
D'après l'analyse univariée, les variables explicatives ayant un effet significatif sur le risque de décès sont :
...
### Stratification sur la FEVG

```{r}
cox_uniIR_FE_sev = coxph(Surv(TEMPS,DC)~ AGE
                           + SEXE
                           + IR,
      data=df[df$FRACTIONCAT == 2,],
      ties = "exact")

cox_uniIR_FE_mod = coxph(Surv(TEMPS,DC)~ AGE
                           + SEXE
                           + IR,
      data=df[df$FRACTIONCAT == 1,],
      ties = "exact")

cox_uniIR_FE_leg = coxph(Surv(TEMPS,DC)~ AGE
                           + SEXE
                           + IR,
      data=df[df$FRACTIONCAT == 0,],
      ties = "exact")

summary(cox_uniIR_FE_sev)
summary(cox_uniIR_FE_mod)
summary(cox_uniIR_FE_leg)
```
...
Aucune variable n'a été mesurée au cours du suivi. Donc il n'y a pas de variables dépendantes du temps.
